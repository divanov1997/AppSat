...
@param time
  ...
  {
    double xx, yy, zz;
    double appcompat; //intermediate variable to be able to let Math.squrt run on android
    double r0, r1, r2, s0, s1, s2; //using doubles to avoid working with "array[x]" form of variables
    if (aRect[0] == 0. && aRect[1] == 0. && aRect[2] == 0.) {
      aSpher[0] = 0; aSpher[1] = 0; aSpher[2] = 0; return;
    }
    //original line for aSpher[2]
    //aSpher[2] = Math.sqrt(aRect[0] * aRect[0] + aRect[1] * aRect[1]
    //				+ aRect[2] * aRect[2]);
    //create variables for holding the array values
    r0 = aRect[0]; r1 = aRect[1]; r2 = aRect[2];
    appcompat = (r0*r0) + (r1*r1) + (r2*r2);
    appcompat = Math.sqrt(appcompat);
    s2 = appcompat;
    /*Original code
    xx = aRect[0] / aSpher[2];
    yy = aRect[1] / aSpher[2];
	...
	*/
    //Slightly modified code to avoid arrays
    xx = r0 / s2;
    yy = r1 / s2;
    zz = r2 / s2;
    s0 = Math.atan2(yy, xx);
    s1 = Math.asin(zz);
    if(s0 < 0.) s0+=2.*Math.PI;
    aSpher [0] = s0;
    aSpher [1] = s1;
    aSpher [2] = s2;
    return;
